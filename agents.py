...
from server.agent_tools import (
    PENDING_ACTION_TOOL_NAMES,
    bind_executive_tools,
    bind_stream_tools,
    command_key_for_tool_call,
)
    ...
    async def _execute_tool_calls(
        ...
        *,
        ...
        for index, function_call in enumerate(function_calls):
            ...
            if name in PENDING_ACTION_TOOL_NAMES:
                hidden_kwargs["command_key"] = command_key_for_tool_call(
...
class GeneralAgentStreamer(Agent):
    ...
    def _prepare_tool_objects_and_python_callables(
        ...
        """Delegate streaming callable construction to the server tool registry."""
        self.available_python_tools, tool_objects = bind_stream_tools(
            policy=policy,
            user_id=self.user_id,
            current_user_email=current_user_email,
        )
